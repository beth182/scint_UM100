...
assert LC_array.shape[1] == LC_array.shape[2]
...
for index, row in grid_coords.iterrows():
    ...
    if np.isnan(LC_array[:, i, j]).all():
        pass
    else:
        assert np.isclose(sum(LC_array[:, i, j]), 1)
    ...
    lc_df = pd.DataFrame.from_dict({'broadleaf': [LC_grid[0]], 'needleleaf': [LC_grid[1]], 'C3': [LC_grid[2]],
     'C4': [LC_grid[3]], 'shrubs': [LC_grid[4]], 'lake': [LC_grid[5]],
     'soil': [LC_grid[6]], 'ice': [LC_grid[7]], 'canyon': [LC_grid[8]],
     'roof': [LC_grid[9]]})
